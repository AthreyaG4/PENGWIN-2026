...
def split_cases(cases: list[Path], anatomy: str) -> dict[str, list[Path]]:
    splits_data = json.loads(SPLITS_FILE.read_text())
    valid_names = splits_data[anatomy]
    by_name = {p.name: p for p in cases}
    return {
        split: [by_name[name] for name in names if name in by_name]
        for split, names in valid_names.items()
    }
...
def process_anatomy(anatomy: str, cases: list[Path], anatomy_dir: Path) -> None:
    ...
    splits = split_cases(cases, anatomy)
