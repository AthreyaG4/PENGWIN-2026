...
def preprocess(
    ...
    shape_before_orient = list(data["image"].shape)
    spacing_before_orient = data["image"].pixdim.tolist()

    data = _orient(data)

    shape_after_orient = list(data["image"].shape)
    spacing_after_orient = data["image"].pixdim.tolist()

    if (
        shape_before_orient != shape_after_orient
        or spacing_before_orient != spacing_after_orient
    ):
        print(
            f"[INFO] Orientation changed shape from {shape_before_orient} to {shape_after_orient} and spacing from {spacing_before_orient} to {spacing_after_orient} for {image_src.name}"
        )
